...
ssh -t -oStrictHostKeyChecking=no -i $(pwd)/.deploy/travis_id_rsa travis@jersey2.rkeplin.com "chown -R travis:33 /opt/apps/rkeplin-blog/application/wp-content/themes/robkeplin-$TRAVIS_BUILD_NUMBER"
...
# Update plugins
ssh -t -oStrictHostKeyChecking=no -i $(pwd)/.deploy/travis_id_rsa travis@jersey2.rkeplin.com 'rkplugin wordpress-seo 12.3'
